...
store_result = function(model){
  model = model
  rmses = calc_rmse(model, data, trn_idx)
  model_names <<- c(model_names, deparse(substitute(model)))
...
calc_rmse = function(model, data, trn_idx) {
...
store_result(exhaustive_search_model)
```
...
```{r}
log_full_interaction_model = lm(log(Bitcoin_price) ~ (. ^ 2) ,  data = train_data)
plots_model(log_full_interaction_model)
store_result(log_full_interaction_model)
```
...
```{r}
log_interaction_reduced_model = lm(log(Bitcoin_price) ~ (. ^ 2) ,  data = train_data)
n = length(resid(log_interaction_reduced_model))
log_int_mod_back_bic = step(log_interaction_reduced_model, direction = "backward", k = log(n), trace = 0)
log_int_mod_back_aic = step(log_interaction_reduced_model, direction = "backward", step=0)
```
...
```{r}
cooksdistance = cooks.distance(log_int_mod_back_bic)
log_int_mod_back_bic_fix = lm(log_int_mod_back_bic,
                   data = data,
...
store_result(log_int_mod_back_bic_fix)
```
```{r}

cooksdistance = cooks.distance(log_int_mod_back_aic)
log_int_mod_back_aic_fix = lm(log_int_mod_back_aic,
                   data = data,
                   subset = cooksdistance < 4 / length(cooksdistance))
plots_model(log_int_mod_back_aic_fix)
plot_prediction(log_int_mod_back_aic_fix, data, trn_idx, log = TRUE)
store_result(log_int_mod_back_aic_fix)
```

```{r}
model_list <- data.frame("model names"=model_names, "num of parameters" = model_params_num, "model_rss" = model_rss, "model_adjr2" = model_adjr2, "model_rmse"=model_rmse, "test_rmse"=test_rmse)
model_lists
```
