...
```{r}
#This breaks up the data into a training and testing set.  The training set will include `r nrow(data_trn)` rows of data, while
...
```{r}
calc_rmse = function(model = gamble_model, data = combined_monthly) {
  trn_idx = sample(nrow(data),nrow(data)/2)
  data_train = data[trn_idx, ]
  data_test = data[-trn_idx, ]
...
```{r}
calc_rmse(model = gamble_model, data)
```

```{r}
calc_rmse(model = autompg_big_mod, data)
```

```{r}

plots_model = function(model = gamble_model){
  par(mfrow = c(1, 2))
