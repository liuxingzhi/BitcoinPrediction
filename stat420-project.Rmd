...
plot(Price ~ Date, data=Dow_Jones_Industrial_Average)
```
```{r}
...
plot(Price ~ Date, data=NYSE_Composite_Index)
```


```{r}
Apple_Inc_company_stock <- read_csv("AAPL Historical Data.csv")
Apple_Inc_company_stock$Date = as.Date(Apple_Inc_company_stock2$Date, format="%b %d, %Y")
plot(Price ~ Date, data=Apple_Inc_company_stock)
```
...
select Bitcoin.Date,
       ...
       exchange.Yuan_USD_exchange_rate,
       ...
       Bitcoin.year
from Bitcoin
         left outer join US_dollar_yuan_exchange_rate as exchange
                         on exchange.Date = Bitcoin.Date
         left outer join Dow_Jones_Industrial_Average as DJIA
                         on DJIA.Date = exchange.Date
         left outer join NYSE_Composite_Index as NYSE
                         on NYSE.Date = DJIA.Date
...
where Bitcoin.Date between {c (1.0, as.Date('2010-09-01'))[2]} and {c (1.0, as.Date('2020-04-30'))[2]}
...
```
```{r}
combined_weekday = na.omit(combined_daily) 
# combined_weekday
combined_weekday
combined_weekday
```
