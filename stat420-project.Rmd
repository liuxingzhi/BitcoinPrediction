...
plot(interest_rate ~ Date, data=The_interest_rate_for_the_United_States)
```
...
plot(Yuan_USD_exchange_rate ~ Date, data = US_dollar_yuan_exchange_rate)
```
...
```{r}
Dow_Jones_Industrial_Average <- read_csv("Dow Jones Industrial Average.csv")
plot(Adj_Close ~ Date, data=Dow_Jones_Industrial_Average)
```
```{r}
NYSE_Composite_Index <- read_csv("NYSE Composite Index.csv")
plot(Adj_Close ~ Date, data=NYSE_Composite_Index)
```
...
plot(Adj_Close ~ Date, data=Apple_Inc_company_stock)
```
